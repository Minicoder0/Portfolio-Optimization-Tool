...
function passwordStrength(pwd: string): { label: string; pct: number; color: string } {
  if (pwd.length < 4) return { label: "Weak", pct: 20, color: "bg-red-500" };
  if (pwd.length < 8) return { label: "Fair", pct: 50, color: "bg-amber-500" };
  ...
  return { label: "Moderate", pct: 70, color: "bg-amber-400" };
}
...
export function SignupForm() {
  ...
  const upsertUserProfile = async (uid: string, userEmail: string, userName: string) => {
    await setDoc(
      doc(db, "users", uid),
      { name: userName, email: userEmail, createdAt: serverTimestamp(), updatedAt: serverTimestamp(), modePreference: "beginner" },
      { merge: true }
    );
  };
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await upsertUserProfile(credential.user.uid, email, name);
  ...
  const onGoogle = async () => {
    ...
    try {
      ...
      await upsertUserProfile(credential.user.uid, credential.user.email ?? "", credential.user.displayName ?? "");
      await setSessionAndRedirect();
    } catch {
    ...
    <div className="flex min-h-[calc(100vh-200px)] items-center justify-center px-4">
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-400">Start building smarter portfolios today</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          {password.length > 0 && (
            <div className="space-y-1">
              <div className="h-1 w-full rounded-full bg-surface-bright overflow-hidden">
